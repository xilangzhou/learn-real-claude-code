...
const SHUTDOWN_STEPS = [
  ...
  { title: "Worker Decides", desc: "The worker can accept or reject. It is a typed coordination request, not an unstructured interruption." },
  { title: "Approved", desc: "The same request_id appears in the response, so the runtime can match the outcome cleanly." },
];

// -- Plan approval protocol step definitions --
const PLAN_STEPS = [
  { title: "Plan Approval", desc: "Workers can request approval before implementation, and the runtime tracks the request lifecycle." },
  { title: "Submit Plan", desc: "The worker designs a plan and sends it to the leader for review." },
...
export default function TeamProtocols({ title }: { title?: string }) {
  ...
  const rightLabel = "Worker";

  return (
    <section className="space-y-4">
      <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">
        {title || "Typed Team Protocols"}
      </h2>
