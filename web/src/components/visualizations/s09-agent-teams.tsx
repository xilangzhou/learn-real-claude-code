...
// Step configuration
const STEPS = [
  { title: "The Team", desc: "Teams use a lead-worker pattern. Each named worker has its own mailbox." },
  { title: "Lead Assigns Work", desc: "Communication is async: the lead sends an explicit message to a specific worker." },
  { title: "Read Inbox", desc: "Workers poll their inbox before the next model call so new messages become usable context." },
  { title: "Independent Work", desc: "Each worker runs its own agent loop and keeps its own state." },
  { title: "Pass Result", desc: "Results return through the same mailbox mechanism. Collaboration stays inspectable." },
  { title: "Feedback Loop", desc: "The mailbox pattern supports many topologies without pretending workers share hidden thought." },
  { title: "Explicit Coordination", desc: "Clear message passing makes the team runtime simple, debuggable, and observable." },
];
