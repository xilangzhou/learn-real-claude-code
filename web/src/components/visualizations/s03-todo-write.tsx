...
// Reminder timer value at each step (out of 3)
const REMINDER_TIMER_PER_STEP = [0, 1, 2, 3, 0, 0, 0];
const REMINDER_THRESHOLD = 3;

// Whether the reminder fires at this step
const REMINDER_FIRES_PER_STEP = [false, false, false, true, false, false, false];

// Step annotations
const STEP_INFO = [
  { title: "Session Checklist", desc: "Session state gives the model a visible short-horizon plan. All tasks start as pending." },
  { title: "Round 1 -- Idle", desc: "The model does work but does not update the checklist. The reminder counter increments." },
  { title: "Round 2 -- Still Idle", desc: "Two rounds without checklist progress. Pressure builds." },
  { title: "Reminder Fires", desc: "Threshold reached. The harness injects a reminder to re-engage with the visible plan." },
  { title: "Task Complete", desc: "The model finishes the active task. Updating session state resets the counter." },
  { title: "Self-Directed", desc: "Once the pattern is learned, the model updates session state voluntarily." },
  { title: "Reliable Progress", desc: "Visible session state plus gentle reminders improves completion reliability." },
];
...
function ReminderGauge({ value, max, firing }: { value: number; max: number; firing: boolean }) {
...
export default function TodoWrite({ title }: { title?: string }) {
  ...
  const nagValue = REMINDER_TIMER_PER_STEP[currentStep];
  const nagFires = REMINDER_FIRES_PER_STEP[currentStep];
  ...
  return (
    <section className="min-h-[500px] space-y-4">
      <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">
        {title || "Session State Reminder Loop"}
      </h2>

      <div className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        {/* Nag gauge + nag message */}
        <div className="mb-4 space-y-2">
          <ReminderGauge value={nagValue} max={REMINDER_THRESHOLD} firing={nagFires} />
          ...
          <AnimatePresence>
            {nagFires && (
              ...
              >
                SYSTEM: "You still have visible pending work. Update the checklist and continue."
              </motion.div>
            )}
          </AnimatePresence>
        </div>
