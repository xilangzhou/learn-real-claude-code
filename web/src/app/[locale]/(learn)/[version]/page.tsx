...
import { getVersionMetaText } from "@/lib/version-i18n";
...
export default async function VersionPage({
  ...
  const metaText = getVersionMetaText(locale, version);
    ...
    <div className="mx-auto max-w-3xl space-y-10 py-4">
      ...
      <header className="space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <h1 className="text-2xl font-bold sm:text-3xl">{metaText.title}</h1>
          {layer && (
          ...
          >
            ...
            <div>
              ...
              <div className="font-medium">
                {prevVersion} - {getVersionMetaText(locale, prevVersion).title}
              </div>
            </div>
          </Link>
        ) : (
        ...
        {nextVersion ? (
          ...
          >
            <div>
              ...
              <div className="font-medium">
                {getVersionMetaText(locale, nextVersion).title} - {nextVersion}
              </div>
            </div>
            <span className="transition-transform group-hover:translate-x-1">
              &rarr;
